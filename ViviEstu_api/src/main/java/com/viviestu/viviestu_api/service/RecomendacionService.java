...
public class RecomendacionService {
    ...
    public List<NotificacionDTO> generarRecomendacionesParaUsuario(Long usuarioId, int topN) {
        ...
        Usuario u = uOpt.get();
        // Requisitos: obtener preferencias del usuario (universidad, presupuesto, transporte)
        String universidad = null;
        Double presupuesto = null;
        String transporte = "";
        if (u.getPreferencias() != null && !u.getPreferencias().isEmpty()) {
            // Tomamos la primera preferencia registrada como preferencia activa
            var p = u.getPreferencias().get(0);
            universidad = p.getUniversidad();
            if (p.getPresupuesto() != null) presupuesto = p.getPresupuesto().doubleValue();
            transporte = p.getTransporte() == null ? "" : p.getTransporte().toLowerCase().trim();
        }

        if (universidad == null || universidad.trim().isEmpty() || presupuesto == null) {
