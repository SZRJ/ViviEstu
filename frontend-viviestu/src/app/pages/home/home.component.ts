...
export class HomeComponent implements OnInit {
  ...
  toggleUserMenu(event: Event) {
    event.stopPropagation(); // Evita que el clic se propague y cierre el menú inmediatamente si tuvieras un listener global
    this.showUserMenu = !this.showUserMenu;
  }
  
  // Navegación
  irAlPerfil() {
    this.router.navigate(['/editar-perfil']); // O '/profile' si prefieres la vista de lectura
  }
  
  irADesactivarCuenta() {
    this.router.navigate(['/profile']); // Asumo que el botón de eliminar está en el perfil de lectura
  }
